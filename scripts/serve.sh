...
if ! $DEV_MODE; then
    FRONTEND_SECRET="$(python3 -c 'import secrets; print(secrets.token_hex(16))')"
fi
...
(cd backend && NO_COLOR=1 uv run langgraph dev --no-browser --allow-blocking --port "$LANGGRAPH_PORT" $LANGGRAPH_EXTRA_FLAGS > ../logs/langgraph.log 2>&1) &
(cd backend && PYTHONPATH=. uv run uvicorn app.gateway.app:app --host "${GATEWAY_HOST:-127.0.0.1}" --port "$GATEWAY_PORT" $GATEWAY_EXTRA_FLAGS > ../logs/gateway.log 2>&1) &
if $DEV_MODE; then
    (cd frontend && pnpm exec next dev --webpack --port "$FRONTEND_PORT" > ../logs/frontend.log 2>&1) &
else
    (
        cd frontend && {
            env BETTER_AUTH_SECRET="$FRONTEND_SECRET" pnpm exec next build &&
            env BETTER_AUTH_SECRET="$FRONTEND_SECRET" pnpm exec next start --port "$FRONTEND_PORT"
