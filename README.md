...
Anaxa 1.0 默认按开源开发版发布：下载源码后，本地初始化、本地运行、前端配置模型，方便直接使用和二次开发。默认不启用 UI 密码；它适合本机或可信局域网环境，不是公网生产部署模式。
...
缺什么它会直接告诉你安装方式。`tectonic` 是可选项，只影响 LaTeX PDF 生成的稳定性。
...
生成的配置文件都在 `.gitignore` 中，不会作为开源代码提交。真实 API Key 不需要手动写文件，后续在前端配置即可。
...
前端“功能”页只读展示当前可用的 Agents、MCP tools 和 Skills。它不提供新增、删除、编辑、启用或停用入口。

### 高级用户：配置文件

`config.yaml` 和 `extensions_config.json` 仍然存在，适合脚本化管理或深度定制：
...
如果检查失败，先处理输出中列出的路径，再 push 到公开仓库。这个命令不会删除任何本地文件。

### 公网部署密码

开源开发版默认不设置 UI 密码。如果你要把服务暴露到公网，再设置 `MEDRIX_FLOW_ENV=production`、`MEDRIX_FLOW_UI_PASSWORD` 和可选的 `MEDRIX_GATEWAY_ADMIN_TOKEN`。

### 兼容技术标识

Anaxa 1.0 已完成用户可见品牌迁移，但为了避免破坏已有脚本、数据目录、环境变量和 import 路径，以下技术标识仍保留：
